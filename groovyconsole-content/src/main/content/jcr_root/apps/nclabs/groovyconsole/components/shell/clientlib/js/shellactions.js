(function ($, document, graniteAuthor) {
    ...
    function writeMessageBox(data) {
        var messageType = data.failed ? "error" : "success";

        var timing = data.executionTime + " ms";
        let message = "";
        if (data.error) {
            message += "<br />";
            message += data.error;
    ...
    function writeScriptExecutionResults(data) {
        ...
        var resultbox = document.getElementById("groovy-results-panel-el");
        ...
        var alertBox = writeMessageBox(data);
